...
int PCA9955A::number_of_ports(void) { return (n_of_ports); }

unsigned char PCA9955A::gradation_group[16] = {0xFF};

void PCA9955A::gradation_setting(int group, char ramp_rate, char step_time,
                                 char hold_cntl, char iref)
{
    char data[5] = {
        (char)((RAMP_RATE_GRP0 + (group * GRAD_GROUP_OFFSET)) | AUTO_INCREMENT),
        ramp_rate, step_time, hold_cntl, iref};
...
void PCA9955A::gradation_group_setting(unsigned char group, unsigned char port)
{
    short g[4] = {0x0000};

    port &= 0xF;
    gradation_group[port] = (group < 4) ? group : GRADATION_NOGROUP;

    for (int i = 0; i < 16; i++) {
        if (gradation_group[i] != GRADATION_NOGROUP)
...
{
    for (int i = 0; i < 16; i++) {
        gradation_group[i] = GRADATION_NOGROUP;
    }

    group_selector(0, 0, 0, 0);
}

float PCA9955A::gradation_ramp_setting(char group, float cycle,
...
#define IREF_GRP 4
    ...
    float hold_time[] = {0.00, 0.25, 0.50, 0.75, 1.00, 2.00, 4.00, 6.00};
    ...
    if (ramp_flag == NO_RAMP) {
        ...
        ramp_time = (int)((cycle - (hold_time[flag_on] + hold_time[flag_off])) /
                          ((ramp_flag == RAMP_UP_DOWN) ? 2.0 : 1.0) * 1000);
    ...
    //    required_steps, iref ); printf( "prescaler = %d, mult_factor=%d,
    ...
    return ((((((float)(((iref + 1) / iref_inc) * mult_factor)) *
               (prescaler ? 8.0 : 0.5)) *
              ((ramp_flag == RAMP_UP_DOWN) ? 2.0 : 1.0))) *
                0.001 +
            (hold_time[flag_on] + hold_time[flag_off]));
}
