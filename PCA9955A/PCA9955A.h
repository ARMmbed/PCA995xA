...
/** PCA9955A class
 *
 *  This is a driver code for the PCA9955A 16-channel Fm+ I2C-bus 57mA/20V
 * constant current LED driver. This class provides interface for PCA9955A
 * operation and accessing its registers. Detail information is available on
 * next URL.
 *    http://www.nxp.com/products/interface_and_connectivity/i2c/i2c_led_display_control/PCA9955ATW.html
 *
 *  Next sample code shows operation based on low-level-API (operated by just
 * device instane)
 *
 *  Example:
 *  @code
 ...
 *  #include "mbed.h"
 *  #include "PCA9955A.h"
 *  PCA9955A    led_cntlr( p28, p27, 0x02 );    //  SDA, SCL,
 * Slave_address(option)
 *
 *  int main()
 *  {
 *      led_cntlr.current( ALLPORTS, 1.0 ); //  Set all ports output current
 * 100%
 *
 *      while(1) {
 ...
 *
 *  PCA9955A    led_cntlr( p28, p27, 0x02 );    //  SDA, SCL,
 * Slave_address(option) LedPwmOutCC led0( led_cntlr, L0  );         //
 * Instance for LED0 pin LedPwmOutCC led1( led_cntlr, L1  );         //
 * Instance for LED1 pin LedPwmOutCC led2( led_cntlr, L2  );         //
 * Instance for LED2 pin
 *
 *  int main()
 *  {
...
public:
    ...
    /** Create a PCA9955A instance connected to specified I2C pins with
     * specified address
     *
     ...
     */
    PCA9955A(PinName i2c_sda, PinName i2c_scl,
    ...
    /** Set all output port duty-cycle, specified as a percentage (array of
     * float)
     *
     * @param vp    Aray to floating-point values representing the output
     * duty-cycle, specified as a percentage. The value should lie between 0.0f
     ...
     * User can set the gradation by specifying cycle, hold-ON/OFF time and
     * ramp-up/down enabling. All register values are calcurated automatically
     * to have fine gradation setting. In this routine, the IREF (current)
     * setting is fixed to maximum (i.e. 0xFF)
     *
     * @param group     Group selector
     * @param cycle     Ramp rate register (RAMP_RATE_GRPn) value
     * @param flag_on   Holding time for ON can be selected from available
     * constants. Those are 0, 0.25, 0.5, 0.75, 1, 2, 4 and 6 seconds. Use one
     * of next words: HOLD_0_00_SEC, HOLD_0_25_SEC, HOLD_0_50_SEC,
     * HOLD_0_75_SEC, HOLD_1_00_SEC, HOLD_2_00_SEC, HOLD_4_00_SEC or
     * HOLD_6_00_SEC
     * @param flag_off  Holding time for OFF can be selected from available
     * constants. Those are 0, 0.25, 0.5, 0.75, 1, 2, 4 and 6 seconds. Use one
     * of next words: HOLD_0_00_SEC, HOLD_0_25_SEC, HOLD_0_50_SEC,
     * HOLD_0_75_SEC, HOLD_1_00_SEC, HOLD_2_00_SEC, HOLD_4_00_SEC or
     * HOLD_6_00_SEC
     * @param ramp_flag Choose one from next words : NO_RAMP, RAMP_UP_ONLY,
     * RAMP_DOWN_ONLY or RAMP_UP_DOWN
     *
     * @return  calculated actual time for one cycle (in seconds)
     *
     * @see gradation_setting()
     */
    float gradation_ramp_setting(char group, float cycle, unsigned char flag_on,
                                 unsigned char flag_off, int ramp_flag);

    /** Gradation control: Port assignment to group
     *
     * Each ports which is going to use in gradation control should be assigned
     * to group. This function help to assign the port to 1 of 4 groups.
     *
     * @param group     Target group
     * @param port      A output port which is being assigned to the target
     * group
     */
    void gradation_group_setting(unsigned char group, unsigned char port);
     ...
     */
    void gradation_setting(int group, char ramp_rate, char step_time,
                           char hold_cntl, char iref);
...
private:
    ...
    static unsigned char gradation_group[];
};

/** Constants for Gradation control */
const unsigned char GRADATION_CONTINUOUS = 1;
const unsigned char GRADATION_NOGROUP = 0xFF;
